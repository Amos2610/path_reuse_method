...
class Encoder(Node):
    ...
    def cross_three_dim(self, vector1, vector2):
        """
        3次元の外積演算を行う関数
        Args:
            vector1: ベクトル1
    ...
    def convert_data(self, matrix):
        ...
        output = '\n'.join(str_list)
        self.get_logger().info(f"matrix: {matrix}, output: {output}")
        return output
    ...
    def generate_pathseed(self, directory):
        """
        パスシードを生成する関数
        Returns:
            dict: パスシードデータ
        """
        self.get_logger().info(f"Generating pathseed from directory: {directory}")
        ...
        if pathseed_data is not None:
            ...
            try:
                ...
                self.get_logger().info(f"データが {file_name} に書き込まれました。")
            except IOError as e:
                self.get_logger().error(f"ファイルの書き込み中にエラーが発生しました: {e}")

            return pathseed_data
        else:
            self.get_logger().error("パスシードの生成に失敗しました。")
            return None
...
def main(args=None):
    ...
    encoder.generate_pathseed(directory)
