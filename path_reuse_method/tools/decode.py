...
class Decoder(Node):
    ...
    def write_data(self, write_data, write_file):
        ...
        # データをフォーマット
        write_data = self.formated_data(write_data)
        # データをファイルに書き込む
        with open(write_file, "w") as f:
            f.write(write_data)
        self.get_logger().info(f"Data has been written to {write_file}")
...
def main(args=None):
    ...
    pathseed_file = 'pathseed.txt'  # サンプルのファイルパス
    start_joint_values = [0.0, 0.0, 0.0]  # サンプルの開始関節角度
    goal_joint_values = [1.0, 1.0, 1.0]  # サンプルのゴール関節角度

    decoder.generate_path(pathseed_file, start_joint_values, goal_joint_values)
